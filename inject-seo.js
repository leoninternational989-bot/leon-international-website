...
pageData.forEach(item => {
    ...
    // 1. Regex to remove old FaqAccordion block entirely.
    // We match from `<div className="mt-24 pt-16` down to its closing </div> </div> </div> block.
    // A quick hack is to replace the entire <FaqAccordion /> tag and surrounding generic divs 
    // by finding <FaqAccordion and deleting its contents. But it's safer to just replace <FaqAccordion... /> with empty
    content = content.replace(/import FaqAccordion from '@\/components\/ui\/FaqAccordion';\n/g, '');
    content = content.replace(/<div className="mt-24[^>]*>[\s\S]*?<FaqAccordion[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/, '');

    // Also remove generic FaqAccordion alone if it exists differently
    content = content.replace(/<FaqAccordion[\s\S]*?\/>/g, '');

    // 2. Add Imports at the top (after last import)
...
import BreadcrumbSchema from '@/components/seo/BreadcrumbSchema';\n`;
    ...
    // 3. Prepare the new schema strings
    const faqString = JSON.stringify(item.faqs, null, 2);

    // We inject before closing </main>
    const injectedCode = `
            <FAQSchema faqs={${faqString}} />
            <FAQSection faqs={${faqString}} />
