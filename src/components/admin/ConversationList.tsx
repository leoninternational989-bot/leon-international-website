...
interface ConversationListProps {
  ...
  loading: boolean;
  // Multi-select props (all optional for backward compat)
...
export default function ConversationList({
  ...
  }, [selectionMode, onToggleSelect, onSelect]);
          ...
          conversations.map((conv) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-gray-50 transition-colors select-none ${
                  isSelected
                    ? 'bg-yellow-50 border-l-2 border-l-yellow-400'
                    : selectedId === conv.id
                      ? 'bg-blue-50/60 border-l-2 border-l-[#2E86C1]'
                      : 'hover:bg-gray-50'
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  {selectionMode && (
                    <div className="flex items-center justify-center shrink-0 mt-1.5">
                      <input
                        ...
                        checked={isSelected}
                        readOnly
                        className="w-4 h-4 rounded accent-yellow-400 pointer-events-none"
                      />
                    </div>
                  )}
